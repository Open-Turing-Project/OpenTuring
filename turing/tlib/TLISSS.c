...
typedef	TLaddressint	TL_TLI___x555[21];
typedef	TL_TLI___x555	TL_TLI_ArgList;
typedef	TLchar	TL_TLI___x558[16];
typedef	TL_TLI___x558	TL_TLI___x557[3];
typedef	struct TL_TLI_StreamEntryType	TL_TLI___x559[33];
extern TL_TLI___x559	TL_TLI_TLIS;
...
typedef	TLchar	TL_TLI___x563[42];
extern TL_TLI___x563	TL_TLI_TLIPXL;
typedef	TLchar	TL_TLI___x564[44];
extern TL_TLI___x564	TL_TLI_TLIPXN;
...
void TL_TLI_TLISSS (status, streamNo, ioMode)
TLint4	*status;
TLint4	streamNo;
TL_Cint	ioMode;
{
    TL_TLI_TLISS((TLint4) streamNo, (TL_Cint) ioMode);
    (*status) = 0;
}
