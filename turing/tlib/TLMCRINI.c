...
typedef	struct TL_TLM_TL_ConditionDescriptor	TL_TLM___x364[65535];
...
void TL_TLM_TLMCRINI (name, cdArray, cdArraySize, md)
TLaddressint	name;
TL_TLM___x364	*cdArray;
TLnat4	cdArraySize;
struct TL_TLM_TL_MonitorDescriptor	*md;
{
    {
	...
	for (i = cdArraySize; i >= 1; i--) {
	    TLBINDREG((*cd), struct TL_TLM_TL_ConditionDescriptor);
	    cd = &((*(cdArray))[i - 1]);
	    (*cd).signalQ.head = (struct TL_TL_ProcessDescriptor *) 0;
	    (*cd).signalQ.tail = (struct TL_TL_ProcessDescriptor *) 0;
	    (*cd).md = md;
	    if (cdArraySize == 1) {
		(*cd).index = 0;
	    } else {
	    ...
	    (*cd).name = (TLaddressint) name;
	    (*cd).nextCondition = md->firstCondition;
	    (*cd).prevCondition = (struct TL_TLM_TL_ConditionDescriptor *) 0;
	    {
		TLaddressint	__x365;
		__x365 = (TLaddressint) ((unsigned long)&((*cd)));
		md->firstCondition = (* (struct TL_TLM_TL_ConditionDescriptor * *) &__x365);
	    };
	    if (((*cd).nextCondition) != ((struct TL_TLM_TL_ConditionDescriptor *) 0)) {
		{
		    TLaddressint	__x367;
		    __x367 = (TLaddressint) ((unsigned long)&((*cd)));
		    (*cd).nextCondition->prevCondition = (* (struct TL_TLM_TL_ConditionDescriptor * *) &__x367);
		};
	    };
	};
    };
