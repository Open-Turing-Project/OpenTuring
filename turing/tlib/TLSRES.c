...
typedef	TLchar	TL_TLS___x432[3];

extern void TL_TLS_TLSBX ();
typedef	TLchar	TL_TLS___x433[3];

extern void TL_TLS_TLSBXX ();

extern void TL_TLS_TLSCXX ();
void TL_TLS_TLSRES (target, __x446, source, dstIndex, rplLength)
TLchar	target[];
TLint4	__x446;
TLstring	source;
TLint4	dstIndex;
TLint4	rplLength;
{
    ...
    extraSpace = TL_TLS_TLSLEN(source) - rplLength;
    if (extraSpace > 0) {
	TLaddressint	lastAddr;
	if ((tgtLength + extraSpace) > __x446) {
