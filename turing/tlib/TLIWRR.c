...
extern void TL_TLI_TLIFS ();
void TL_TLI_TLIWRR (sizeWritten, writeItem, itemSize, status, streamNo)
TLint4	*sizeWritten;
TLaddressint	writeItem;
TLint4	itemSize;
TLaddressint	status;
TLint2	streamNo;
{
    (*sizeWritten) = (*TL_TLI_TLIFWR)((TLaddressint) writeItem, (TL_Cint) 1, (TL_Cint) itemSize, (TLaddressint) (TL_TLI_TLIS[streamNo - -2].info));
    if ((unsigned long) status != 0) {
	if ((*sizeWritten) == itemSize) {
	    (* (TLint4 *) status) = 0;
	} else {
	    (* (TLint4 *) status) = -1;
	};
