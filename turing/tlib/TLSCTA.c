...
typedef	TLchar	TL_TLS___x376[3];

extern void TL_TLS_TLSBX ();
typedef	TLchar	TL_TLS___x377[3];

extern void TL_TLS_TLSBXX ();
void TL_TLS_TLSCTA (target, __x378, right)
TLchar	target[];
TLint4	__x378;
TLstring	right;
{
    register TLaddressint	dst;
    TLaddressint	firstAddr;
    TLaddressint	lastAddr;
    register TLaddressint	src;
    TLchar	firstChar;
    lastAddr = (TLaddressint) ((unsigned long) ((unsigned long)target) + __x378);
    src = (TLaddressint) ((unsigned long)right);
    firstChar = (* (TLchar *) src);
    if (firstChar == '\0') {
	return;
    };
    src += 1;
	...
	if ((unsigned long) dst >= (unsigned long) lastAddr) {
	    (* (TLchar *) dst) = '\0';
	    (* (TLchar *) firstAddr) = firstChar;
	    TL_TLQUIT("Result of string concatenation assign exceeds maximum length of string", (TLint4) 35);
	};
