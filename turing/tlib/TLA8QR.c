...
typedef	TLnat4	TL_TLA___x467[11];
extern TL_TLA___x467	TL_TLA_TLARS;
...
TLreal8	value;
{
    ...
    if (value <= 0) {
	if (value < 0) {
	    TL_TLQUIT("Negative value passed to Sqrt", (TLint4) 95);
	};
	return (0);
    };
    n = TL_TLA_TLA8GX((TLreal8) value);
    temp = 0.41731 + (0.59016 * TL_TLA_TLA8SX((TLreal8) value, (TLint4) 0));
    if ((n % 2) != 0) {
	temp *= 0.7071067811865475244008443621048490392848;
	n += 1;
    };
    temp = TL_TLA_TLA8SX((TLreal8) temp, (TLint4) (TL_TLA_TLA8GX((TLreal8) temp) + (n / 2)));
    temp += ((TLreal8) value)  / ((TLreal8) temp);
    temp = (0.25 * temp) + (((TLreal8) value)  / ((TLreal8) temp));
    return (0.5 * (temp + (((TLreal8) value)  / ((TLreal8) temp))));
    /* NOTREACHED */
}
