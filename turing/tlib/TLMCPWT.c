...
typedef	struct TL_TLM_TL_ConditionDescriptor	TL_TLM___x363[65535];
...
void TL_TLM_TLMCPWT (waitPriority, cd)
TLnat4	waitPriority;
register struct TL_TLM_TL_ConditionDescriptor	*cd;
{
    ...
    TL_TLKPD->waitParameter = waitPriority;
	...
	register struct TL_TL_ProcessDescriptor	*next;
	register struct TL_TL_ProcessDescriptor	*last;
	next = cd->signalQ.head;
	last = (struct TL_TL_ProcessDescriptor *) 0;
	for(;;) {
	    if (waitPriority < (next->waitParameter)) {
		break;
	    };
	    last = next;
	    next = next->monitorQlink;
	    if (next == ((struct TL_TL_ProcessDescriptor *) 0)) {
		break;
	    };
	};
	TL_TLKPD->monitorQlink = next;
	if (last != ((struct TL_TL_ProcessDescriptor *) 0)) {
	    last->monitorQlink = TL_TLKPD;
	} else {
	    cd->signalQ.head = TL_TLKPD;
	};
    };
