...
typedef	TLchar	TL_TLS___x416[3];

extern void TL_TLS_TLSBX ();
typedef	TLchar	TL_TLS___x417[3];

extern void TL_TLS_TLSBXX ();

extern void TL_TLS_TLSCXX ();
void TL_TLS_TLSMSC (target, source, __x427, dstIndex)
TLstring	target;
TLchar	source[];
TLint4	__x427;
TLint4	dstIndex;
{
    ...
    if (dstIndex > ((TL_TLS_TLSLEN(target) - __x427) + 1)) {
	TL_TLQUIT("Attempt to \"strmove\" past the end of string", (TLint4) 40);
    };
    dst = (TLaddressint) (((unsigned long) ((unsigned long)target) + dstIndex) - 1);
    src = (TLaddressint) ((unsigned long)source);
    lastAddr = (TLaddressint) ((unsigned long)&(source[((__x427 + 0) - 1)]));
    for(;;) {
