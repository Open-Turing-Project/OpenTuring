...
typedef	TLnat4	TL_TLA___x601[11];
extern TL_TLA___x601	TL_TLA_TLARS;
...
extern void TL_TLA_TLAVFS ();
void TL_TLA_TLAVS8 (source, answer, error, quitOnError)
TLstring	source;
TLreal8	*answer;
TLboolean	*error;
TLboolean	quitOnError;
{
    ...
    if (!sawDigit) {
	if (quitOnError) {
	    TL_TLQUIT("String passed to \"strreal\" is not in correct format", (TLint4) 90);
	};
	(*error) = 1;
	return;
    };
    if ((((* (TLchar *) src)) == 'e') || (((* (TLchar *) src)) == 'E')) {
	TLboolean	exponentNegative;
	exponentNegative = 0;
	src += 1;
	sawDigit = 0;
	if (((* (TLchar *) src)) == '-') {
	...
	if (TL_TLECU) {
	    (*error) = 1;
	    if (quitOnError) {
		TL_TLQUIT("String passed to \"strreal\" is too small to convert", (TLint4) 52);
	    };
	} else {
	    (*answer) = 0;
	};
	...
	if (exponent > 310) {
	    if (quitOnError) {
		TL_TLQUIT("String passed to \"strreal\" is too large to convert", (TLint4) 51);
	    };
	    (*error) = 1;
	    return;
	};
    };
    TL_TLX_TLXATF(source, &((*answer)));
}
