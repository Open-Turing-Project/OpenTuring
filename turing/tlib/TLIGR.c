...
typedef	TLaddressint	TL_TLI___x473[21];
typedef	TL_TLI___x473	TL_TLI_ArgList;
typedef	TLchar	TL_TLI___x476[16];
typedef	TL_TLI___x476	TL_TLI___x475[3];
typedef	struct TL_TLI_StreamEntryType	TL_TLI___x477[33];
extern TL_TLI___x477	TL_TLI_TLIS;
...
typedef	TLchar	TL_TLI___x481[42];
extern TL_TLI___x481	TL_TLI_TLIPXL;
typedef	TLchar	TL_TLI___x482[44];
extern TL_TLI___x482	TL_TLI_TLIPXN;
...
extern void TL_TLI_TLIFS ();
void TL_TLI_TLIGR (getItem, itemSize, streamNo)
...
TLint2	streamNo;
{
    ...
    TL_TLA_TLAVS8(token, &(value), &(error), (TLboolean) 0);
    if (error) {
	TL_TLQUIT("Invalid real number input", (TLint4) 70);
    };
    if (itemSize == 8) {
	(* (TLreal8 *) getItem) = value;
    } else {
	(* (TLreal4 *) getItem) = value;
	if (( fabs((value - ((* (TLreal4 *) getItem))))) > (((TLreal8) ( TLSIMPLEABS(value)))  / ((TLreal8) 1e5))) {
	    TL_TLQUIT("Real value too large for real4 variable", (TLint4) 70);
	};
    };
}
