...
extern void TL_TLB_TLBMFR ();
void TL_TLB_TLBFRU (objAddr, objSize)
TLaddressint	*objAddr;
TLnat4	objSize;
{
    if ((unsigned long) (*objAddr) == 0) {
	TL_TLQUIT("Attempt to free nil pointer", (TLint4) 66);
    } else {
	if ((unsigned long) (*objAddr) == 0xFFFFFFFF) {
	    TL_TLQUIT("Variable has no value", (TLint4) 45);
	};
    };
    TL_TLB_TLBMFR((TLaddressint) (*objAddr));
    (*objAddr) = (TLaddressint) 0;
}
