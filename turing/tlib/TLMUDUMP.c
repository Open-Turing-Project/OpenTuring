...
typedef	struct TL_TLM_TL_ConditionDescriptor	TL_TLM___x330[65535];
...
void TL_TLM_TLMUDUMP () {
    TL_TL_lockStatus_t	y;
    register struct TL_TLM_TL_MonitorDescriptor	*md;
    TL_TLK_TLKLKON(&(TL_TLM_TLMMLL), &(y));
    md = TL_TLM_TLMMLH;
    for(;;) {
	TL_TL_lockStatus_t	x;
	register struct TL_TLM_TL_ConditionDescriptor	*cd;
	if (md == ((struct TL_TLM_TL_MonitorDescriptor *) 0)) {
	    break;
	};
	TL_TLK_TLKLKON(&(md->mQLock), &(x));
	if ((md->entryQ.head) != ((struct TL_TL_ProcessDescriptor *) 0)) {
	    register struct TL_TL_ProcessDescriptor	*pd;
	    TL_TLI_TLISS ((TLint4) 0, (TLint2) 2);
	    TL_TLI_TLIPS ((TLint4) 0, "\n****** Entering Monitor `", (TLint2) 0);
	    TL_TLI_TLIPS ((TLint4) 0, ((* (TLstring *) (md->name))), (TLint2) 0);
	    TL_TLI_TLIPS ((TLint4) 0, "\'\n", (TLint2) 0);
	    pd = md->entryQ.head;
	    for(;;) {
		TL_TLK_TLKUDMPP(pd);
	...
	cd = md->firstCondition;
	for(;;) {
	    struct TL_TL_ProcessDescriptor	*pd;
	    if (cd == ((struct TL_TLM_TL_ConditionDescriptor *) 0)) {
		break;
	    };
	    pd = cd->signalQ.head;
	    if (pd != ((struct TL_TL_ProcessDescriptor *) 0)) {
		TL_TLI_TLISS ((TLint4) 0, (TLint2) 2);
		TL_TLI_TLIPS ((TLint4) 0, "\n****** Waiting for `", (TLint2) 0);
		TL_TLI_TLIPS ((TLint4) 0, ((* (TLstring *) (cd->name))), (TLint2) 0);
		if ((cd->index) != 0) {
		    TL_TLI_TLISS ((TLint4) 0, (TLint2) 2);
		    TL_TLI_TLIPS ((TLint4) 0, "(", (TLint2) 0);
		    TL_TLI_TLIPN ((TLint4) 0, (TLnat4) (cd->index), (TLint2) 0);
		    TL_TLI_TLIPS ((TLint4) 0, ")\'\n", (TLint2) 0);
		} else {
		    TL_TLI_TLISS ((TLint4) 0, (TLint2) 2);
		    TL_TLI_TLIPS ((TLint4) 0, "\'\n", (TLint2) 0);
    ...
    TL_TLK_TLKLKOFF(&(TL_TLM_TLMMLL), (TL_TL_lockStatus_t) y);
}
