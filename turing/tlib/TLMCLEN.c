...
typedef	struct TL_TLM_TL_ConditionDescriptor	TL_TLM___x322[65535];
...
TLnat4 TL_TLM_TLMCLEN (cd)
struct TL_TLM_TL_ConditionDescriptor	*cd;
{
    register struct TL_TL_ProcessDescriptor	*ptr;
    register TLnat4	cnt;
    ptr = cd->signalQ.head;
    cnt = 0;
    for(;;) {
	if (ptr == ((struct TL_TL_ProcessDescriptor *) 0)) {
	    break;
	};
	ptr = ptr->monitorQlink;
	cnt += 1;
    };
    return (cnt);
    /* NOTREACHED */
