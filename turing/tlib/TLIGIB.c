...
typedef	TLaddressint	TL_TLI___x463[21];
typedef	TL_TLI___x463	TL_TLI_ArgList;
typedef	TLchar	TL_TLI___x466[16];
typedef	TL_TLI___x466	TL_TLI___x465[3];
typedef	struct TL_TLI_StreamEntryType	TL_TLI___x467[33];
extern TL_TLI___x467	TL_TLI_TLIS;
...
typedef	TLchar	TL_TLI___x471[42];
extern TL_TLI___x471	TL_TLI_TLIPXL;
typedef	TLchar	TL_TLI___x472[44];
extern TL_TLI___x472	TL_TLI_TLIPXN;
...
extern void TL_TLI_TLIFS ();
void TL_TLI_TLIGIB (upperBound, lowerBound, getItem, itemSize, streamNo)
TLint4	upperBound;
TLint4	lowerBound;
...
TLint2	streamNo;
{
    ...
    TL_TLA_TLAVSI(token, (TLnat1) 10, &(value), &(error), (TLboolean) 0);
    if (error) {
	TL_TLQUIT("Invalid integer input", (TLint4) 70);
    };
    if ((value < lowerBound) || (value > upperBound)) {
	{
	    TLstring	__x479;
	    TL_TLS_TLSVIS((TLint4) upperBound, (TLint4) 1, (TLint4) 10, __x479);
	    {
		TLstring	__x478;
		TL_TLS_TLSVIS((TLint4) lowerBound, (TLint4) 1, (TLint4) 10, __x478);
		{
		    TLstring	__x477;
		    TL_TLS_TLSCAT("Integer input not in range \'", __x478, __x477);
		    {
			TLstring	__x476;
			TL_TLS_TLSCAT(__x477, "\' to \'", __x476);
			{
			    TLstring	__x475;
			    TL_TLS_TLSCAT(__x476, __x479, __x475);
			    {
				TLstring	__x474;
				TL_TLS_TLSCAT(__x475, "\'", __x474);
				TL_TLQUIT(__x474, (TLint4) 70);
			    };
			};
		    };
		};
    ...
    if (itemSize == 4) {
	(* (TLint4 *) getItem) = value;
    } else {
	if (itemSize == 2) {
	    (* (TLint2 *) getItem) = value;
	} else {
	    (* (TLint1 *) getItem) = value;
	};
    };
}
