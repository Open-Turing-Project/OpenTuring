...
typedef	TLchar	TL_TLS___x468[3];

extern void TL_TLS_TLSBX ();
typedef	TLchar	TL_TLS___x469[3];

extern void TL_TLS_TLSBXX ();

extern void TL_TLS_TLSCXX ();
TLint4 TL_TLS_TLSVSI (source, base)
TLstring	source;
TLint4	base;
{
    TLint4	answer;
    TLboolean	error;
    if ((base < 2) || (base > 36)) {
	TL_TLQUIT("Illegal conversion base passed to \"strint\"", (TLint4) 106);
    };
    TL_TLA_TLAVSI(source, (TLnat1) base, &(answer), &(error), (TLboolean) 1);
    return (answer);
    /* NOTREACHED */
}
