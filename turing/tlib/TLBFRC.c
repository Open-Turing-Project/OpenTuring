...
extern void TL_TLB_TLBMFR ();
void TL_TLB_TLBFRC (ptr, objSize)
register struct TL_TLB_CheckedPointer	*ptr;
TLnat4	objSize;
{
    TLBINDREG((*objTimestamp), TL_TLB_Timestamp);
    if ((unsigned long) ((*ptr).objAddress) == 0) {
	TL_TLQUIT("Attempt to free nil pointer", (TLint4) 66);
    } else {
	if ((unsigned long) ((*ptr).objAddress) == 0xFFFFFFFF) {
	    TL_TLQUIT("Variable has no value", (TLint4) 45);
	};
    };
    objTimestamp = ((unsigned long) ((*ptr).objAddress) - sizeof(TL_TLB_Timestamp));
    if (((*ptr).timestamp) != (*objTimestamp)) {
	TL_TLQUIT("Collection element has already been freed", (TLint4) 27);
    };
    (*objTimestamp) = 0;
    (*ptr).timestamp = 0;
    (*ptr).objAddress = (TLaddressint) 0;
    TL_TLB_TLBMFR((TLaddressint) ((unsigned long)&((*objTimestamp))));
}
