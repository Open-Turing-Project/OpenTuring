...
typedef	TLnat4	TL_TLA___x401[11];
extern TL_TLA___x401	TL_TLA_TLARS;
extern TLint4	TL_TLA_TLA8HI;
extern TLint4	TL_TLA_TLA8LO;
void TL_TLA_TLAV8D (value, hiOrder, loOrder)
TLreal8	value;
TLnat4	*hiOrder;
TLnat4	*loOrder;
{
    if (TL_TLA_TLA8HI != TL_TLA_TLA8LO) {
	typedef	TLnat4	__x410[2];
	typedef	__x410	LongArray;
	(*hiOrder) = (* (LongArray *) &value)[TL_TLA_TLA8HI];
	(*loOrder) = (* (LongArray *) &value)[TL_TLA_TLA8LO];
    } else {
	TL_TLQUIT("TLAV8D unimplemented", (TLint4) 60);
    };
}
