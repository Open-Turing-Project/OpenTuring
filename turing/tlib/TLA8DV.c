...
typedef	TLnat4	TL_TLA___x438[11];
extern TL_TLA___x438	TL_TLA_TLARS;
...
TLint4 TL_TLA_TLA8DV (right, left)
TLreal8	right;
TLreal8	left;
{
    TLreal8	quotient;
    quotient = ((TLreal8) left)  / ((TLreal8) right);
    if (quotient < 0) {
	if (quotient < -2147483647) {
	    TL_TLQUIT("Integer overflow in real8 div", (TLint4) 50);
	};
	return (TL_TLA_TLA8CL((TLreal8) quotient));
    } else {
	if (quotient > 0x7FFFFFFF) {
	    TL_TLQUIT("Integer overflow in real8 div", (TLint4) 50);
	};
	return (TL_TLA_TLA8FL((TLreal8) quotient));
    };
    /* NOTREACHED */
}
