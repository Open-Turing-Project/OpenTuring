...
typedef	struct TL_TLM_TL_ConditionDescriptor	TL_TLM___x391[65535];
...
void TL_TLM_TLMRFIN (md)
register struct TL_TLM_TL_MonitorDescriptor	*md;
{
    struct TL_TLM_TL_ConditionDescriptor	*cp;
    TL_TL_lockStatus_t	x;
    struct TL_TLM_TL_MonitorDescriptor	*next;
    struct TL_TLM_TL_MonitorDescriptor	*prev;
    if (((md->reEntryQ.head) != ((struct TL_TL_ProcessDescriptor *) 0)) || ((md->entryQ.head) != ((struct TL_TL_ProcessDescriptor *) 0))) {
	if ((md->reEntryQ.head) != ((struct TL_TL_ProcessDescriptor *) 0)) {
	    TL_TLQUIT("TLMRFIN: LIBRARY ABORT - monitor reEntryQ was not nil", (TLint4) 111);
	} else {
	    TL_TLQUIT("TLMRFIN: LIBRARY ABORT - monitor entryQ was not nil", (TLint4) 111);
	};
    };
    cp = md->firstCondition;
    for(;;) {
	if (cp == ((struct TL_TLM_TL_ConditionDescriptor *) 0)) {
	    break;
	};
	if ((cp->signalQ.head) != ((struct TL_TL_ProcessDescriptor *) 0)) {
	    TL_TLQUIT("TLMRFIN: LIBRARY ABORT - Condition queue was not nil", (TLint4) 111);
	};
	cp = cp->nextCondition;
    };
    TL_TLK_TLKLKON(&(TL_TLM_TLMMLL), &(x));
    next = md->nextMonitor;
    prev = md->prevMonitor;
    if (next != ((struct TL_TLM_TL_MonitorDescriptor *) 0)) {
	next->prevMonitor = prev;
    };
    if (prev == ((struct TL_TLM_TL_MonitorDescriptor *) 0)) {
	TL_TLM_TLMMLH = next;
    } else {
	prev->nextMonitor = next;
    };
    TL_TLK_TLKLKOFF(&(TL_TLM_TLMMLL), (TL_TL_lockStatus_t) x);
}
