...
typedef	struct TL_TLM_TL_ConditionDescriptor	TL_TLM___x381[65535];
...
void TL_TLM_TLMIPINI (md, intrNo, procAddr)
register struct TL_TLM_TL_MonitorDescriptor	*md;
TLint4	intrNo;
TLaddressint	procAddr;
{
    if (!(md->deviceMonitor)) {
	TL_TLQUIT("TLMIPINI - LIBRARY ABORT : Interrupt procedure is not in a device monitor", (TLint4) 21);
    };
    TL_TLK_TLKIPINI((TLnat4) intrNo, (TLaddressint) procAddr, (TLint4) (md->monitorPriority));
}
