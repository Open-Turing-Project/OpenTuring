...
typedef	TLaddressint	TL_TLI___x478[21];
typedef	TL_TLI___x478	TL_TLI_ArgList;
typedef	TLchar	TL_TLI___x481[16];
typedef	TL_TLI___x481	TL_TLI___x480[3];
typedef	struct TL_TLI_StreamEntryType	TL_TLI___x482[33];
extern TL_TLI___x482	TL_TLI_TLIS;
...
typedef	TLchar	TL_TLI___x486[42];
extern TL_TLI___x486	TL_TLI_TLIPXL;
typedef	TLchar	TL_TLI___x487[44];
extern TL_TLI___x487	TL_TLI_TLIPXN;
...
extern void TL_TLI_TLIFS ();
void TL_TLI_TLIGS (itemSize, getItem, streamNo)
TLint4	itemSize;
...
TLint2	streamNo;
{
    ...
    lastAddr = (TLaddressint) ((unsigned long) dst + itemSize);
    if ((*stream).atEof) {
	(* (TLchar *) dst) = '\0';
	TL_TLQUIT("Attempt to read past eof", (TLint4) 80);
    };
    for(;;) {
	...
	if ((((ch != ((TLnat1) ' ')) && (ch != ((TLnat1) '\t'))) && (ch != ((TLnat1) '\n'))) && (ch != ((TLnat1) '\f'))) {
	    break;
	};
    };
    if (ch == ((TLnat1) '"')) {
	for(;;) {
		...
		(* (TLchar *) dst) = '\0';
		TL_TLQUIT("No terminating quote for quoted string", (TLint4) 70);
	    };
	    if ((unsigned long) dst >= (unsigned long) lastAddr) {
		(* (TLchar *) dst) = '\0';
		TL_TLQUIT("Quoted string too large for string variable", (TLint4) 70);
	    };
	    if (ch == ((TLnat1) '\\')) {
		ch = (*TL_TLI_TLIFGC)((TLaddressint) ((*stream).info));
		if (ch == -1) {
		    (*stream).atEof = 1;
		    (* (TLchar *) dst) = '\0';
		    TL_TLQUIT("Unexpected end-of-file", (TLint4) 70);
		} else {
		    switch (((TLchar) ch)) {
			case '"':
			case '\'':
			...
			default :
			    {
				{
				    ...
				    {
					TLchar	__x496[2];
					TLCHRTOSTR(((TLchar) ch), __x496);
					TL_TLS_TLSCAT("Illegal extended char \'\\", __x496, __x495);
				    };
				    {
					TLstring	__x494;
					TL_TLS_TLSCAT(__x495, "\'", __x494);
					TL_TLQUIT(__x494, (TLint4) 70);
				    };
				};
			    }
			    break;
	...
	for(;;) {
	    ...
	    if ((unsigned long) dst >= (unsigned long) lastAddr) {
		(* (TLchar *) dst) = '\0';
		TL_TLQUIT("Input string too large for string variable", (TLint4) 70);
	    };
	};
    };
    (* (TLchar *) dst) = '\0';
	...
	for(;;) {
	    ...
	    if (((ch != ((TLnat1) ' ')) && (ch != ((TLnat1) '\t'))) && (ch != ((TLnat1) '\f'))) {
		break;
	    };
	};
	if (ch != ((TLnat1) '\n')) {
