...
TLaddressint TL_TLE_TLECRS (testString, upperBound, abortNo)
TLstring	testString;
TLint4	upperBound;
TLint4	abortNo;
{
    if (TL_TLS_TLSLEN(testString) > upperBound) {
	TL_TLE_TLEABT((TL_Cint) abortNo);
    };
    return (((unsigned long)testString));
    /* NOTREACHED */
}
