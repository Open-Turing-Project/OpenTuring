...
void TL_TLE_TLELF (lineAndFile, fileTable, lineNumber, fileName)
TLnat4	lineAndFile;
TLaddressint	fileTable;
TLnat4	*lineNumber;
TLaddressint	*fileName;
{
    typedef	TLchar	__x371[1];
    struct	FileTable {
    TLnat4	fileCount;
    __x371	fileNames;
};
    TLnat4	fileNumber;
    register TLaddressint	name;
    if ((lineAndFile == 0) || ((unsigned long) fileTable == 0)) {
	(*lineNumber) = 0;
	(*fileName) = (TLaddressint) 0;
	return;
    };
    fileNumber = lineAndFile / 100000;
    if ((fileNumber < 1) || (fileNumber > ((* (struct FileTable *) fileTable).fileCount))) {
	typedef	TLchar	__x374[26];
	typedef	__x374	__x373[1];
	static __x373	errorMsg = 
	    {"<Corrupted line and file>"};
	(*lineNumber) = 0;
	(*fileName) = (TLaddressint) ((unsigned long)errorMsg[0]);
    };
    (*lineNumber) = lineAndFile % 100000;
    name = (TLaddressint) ((unsigned long)(* (struct FileTable *) fileTable).fileNames);
    {
	...
	if (count <= __x376) {
		...
		name += 1;
		if (count == __x376) break;
		count++;
	    }
	};
    };
    (*fileName) = (TLaddressint) name;
}
