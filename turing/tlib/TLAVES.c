...
typedef	TLnat4	TL_TLA___x592[11];
extern TL_TLA___x592	TL_TLA_TLARS;
...
void TL_TLA_TLAVES (source, width, fWidth, eWidth, target, suppress, error)
TLreal8	source;
TLint4	width;
TLint4	fWidth;
TLint4	eWidth;
...
TLboolean	*error;
{
    ...
    TLnat2	fWidthTaken;
    TLboolean	giveDecPt;
    typedef	TLchar	__x614[12];
    __x614	exponentString;
    TLnat2	eWidthNeeded;
    TLnat2	eWidthTaken;
    TLnat2	widthNeeded;
	...
	if (((hiOrder & 2146435072) >> 20) == 2047) {
	    ...
	    if ((mantissa == 0) && (loOrder == 0)) {
		{
		    ...
		    for (__x573 = width; __x573 >= 4; __x573--) {
			(* (TLchar *) dst) = ' ';
			dst += 1;
		    };
		};
		...
		{
		    register TLint4	__x574;
		    for (__x574 = width; __x574 >= 3; __x574--) {
			(* (TLchar *) dst) = ' ';
			dst += 1;
		    };
		};
    ...
    giveDecPt = 1;
    TL_TLA_TLAVRS(fractionString, (TLreal8) source, (TLint4) (fWidth + 1), &(exponent), &(negative), (TLboolean) 1, &((*error)));
    if ((*error)) {
	return;
    };
    if (suppress) {
	src = (TLaddressint) ((unsigned long) ((unsigned long)fractionString) + fWidth);
	{
	    register TLint4	__x581;
	    for (__x581 = fWidth; __x581 >= 1; __x581--) {
		if (((* (TLchar *) src)) != '0') {
		    break;
		};
		src -= 1;
	...
	fWidthTaken = (unsigned long) src - (unsigned long) ((unsigned long)fractionString);
	if (fWidthTaken == 0) {
	    giveDecPt = 0;
	};
	src += 1;
    ...
    {
	TLstring	__x615;
	TL_TLS_TLSVIS((TLint4) ( TLSIMPLEABS(exponent)), (TLint4) 1, (TLint4) 10, __x615);
	TLSTRASS(11, exponentString, __x615);
    };
    eWidthNeeded = TL_TLS_TLSLEN(exponentString);
    if ((eWidthNeeded > eWidth) || suppress) {
	eWidthTaken = eWidthNeeded;
    } else {
	eWidthTaken = eWidth;
    };
    widthNeeded = (2 + fWidthTaken) + eWidthTaken;
    if (negative) {
	widthNeeded += 1;
    };
    if (giveDecPt) {
    ...
    dst = (TLaddressint) ((unsigned long)target);
    {
	register TLint4	__x587;
	for (__x587 = width - widthNeeded; __x587 >= 1; __x587--) {
	    (* (TLchar *) dst) = ' ';
	    dst += 1;
	};
    };
    ...
    {
	register TLint4	__x588;
	for (__x588 = eWidthTaken - eWidthNeeded; __x588 >= 1; __x588--) {
	    (* (TLchar *) dst) = '0';
	    dst += 1;
	};
    };
    src = (TLaddressint) ((unsigned long)exponentString);
    for(;;) {
	(* (TLchar *) dst) = (* (TLchar *) src);
	dst += 1;
	if (((* (TLchar *) src)) == '\0') {
