...
typedef	struct TL_TLM_TL_ConditionDescriptor	TL_TLM___x335[65535];
...
void TL_TLM_TLMDINI (md, name, devicePriority)
register struct TL_TLM_TL_MonitorDescriptor	*md;
TLaddressint	name;
TLint4	devicePriority;
{
    TL_TLM_TLMRINI(md, (TLaddressint) name);
    if (!TL_TLK_TLKDMINI((TLint4) devicePriority)) {
	{
	    TLstring	__x341;
	    TL_TLS_TLSVIS((TLint4) devicePriority, (TLint4) 1, (TLint4) 10, __x341);
	    {
		TLstring	__x339;
		TL_TLS_TLSCAT("Device monitor \'", (* (TLstring *) name), __x339);
		{
		    TLstring	__x338;
		    TL_TLS_TLSCAT(__x339, "\' has invalid priority level (", __x338);
		    {
			TLstring	__x337;
			TL_TLS_TLSCAT(__x338, __x341, __x337);
			{
			    TLstring	__x336;
			    TL_TLS_TLSCAT(__x337, ")", __x336);
			    TL_TLQUIT(__x336, (TLint4) 110);
			};
		    };
		};
	    };
    ...
    md->monitorPriority = devicePriority;
    md->deviceMonitor = 1;
}
