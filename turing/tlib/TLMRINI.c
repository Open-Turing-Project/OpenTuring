...
typedef	struct TL_TLM_TL_ConditionDescriptor	TL_TLM___x332[65535];
...
void TL_TLM_TLMRINI (md, name)
register struct TL_TLM_TL_MonitorDescriptor	*md;
TLaddressint	name;
{
    TL_TL_lockStatus_t	x;
    md->monitorPriority = 0;
    md->deviceMonitor = 0;
    md->mQLock = 0;
    md->entryParameter = 0;
    md->entryQ.head = (struct TL_TL_ProcessDescriptor *) 0;
    md->entryQ.tail = (struct TL_TL_ProcessDescriptor *) 0;
    md->reEntryQ.head = (struct TL_TL_ProcessDescriptor *) 0;
    md->reEntryQ.tail = (struct TL_TL_ProcessDescriptor *) 0;
    md->name = (TLaddressint) name;
    md->firstCondition = (struct TL_TLM_TL_ConditionDescriptor *) 0;
    TL_TLK_TLKLKON(&(TL_TLM_TLMMLL), &(x));
    md->nextMonitor = TL_TLM_TLMMLH;
    md->prevMonitor = (struct TL_TLM_TL_MonitorDescriptor *) 0;
    if (TL_TLM_TLMMLH != ((struct TL_TLM_TL_MonitorDescriptor *) 0)) {
	TL_TLM_TLMMLH->prevMonitor = md;
    };
    TL_TLM_TLMMLH = md;
