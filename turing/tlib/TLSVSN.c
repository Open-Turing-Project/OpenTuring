...
typedef	TLchar	TL_TLS___x472[3];

extern void TL_TLS_TLSBX ();
typedef	TLchar	TL_TLS___x473[3];

extern void TL_TLS_TLSBXX ();

extern void TL_TLS_TLSCXX ();
TLnat4 TL_TLS_TLSVSN (source, base)
TLstring	source;
TLint4	base;
{
    TLnat4	answer;
    TLboolean	error;
    if ((base < 2) || (base > 36)) {
	TL_TLQUIT("Illegal conversion base passed to \"strnat\"", (TLint4) 106);
    };
    TL_TLA_TLAVSN(source, (TLnat1) base, &(answer), &(error), (TLboolean) 1);
    return (answer);
    /* NOTREACHED */
}
