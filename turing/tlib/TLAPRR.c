...
typedef	TLnat4	TL_TLA___x538[11];
extern TL_TLA___x538	TL_TLA_TLARS;
...
TLreal8 TL_TLA_TLAPRR (right, left)
TLreal8	right;
TLreal8	left;
{
    TLreal8	answer;
    ...
    if (left == 0) {
	if (right < 0) {
	    TL_TLQUIT("0.0 raised to negative power", (TLint4) 98);
	};
	if (right == 0) {
	    TL_TLQUIT("0.0 raised to power 0.0", (TLint4) 99);
	};
	return (0);
    } else {
	if (left < 0) {
	    TL_TLQUIT("Attempt to raise a negative number to a real power", (TLint4) 97);
	};
    };
    TL_TLA_TLA8X((TLreal8) (TL_TLA_TLA8LN((TLreal8) left) * right), &(answer), &(error));
    switch (error) {
	case 0:
	    break;
	case 1:
	    {
		TL_TLQUIT("Overflow in real \"**\" operation", (TLint4) 51);
	    }
	    break;
	case 2:
	    {
		if (TL_TLECU) {
		    TL_TLQUIT("Underflow in real \"**\" operation", (TLint4) 52);
		} else {
		    return (0);
		};
	    }
