...
typedef	TLnat4	TL_TLA___x552[11];
extern TL_TLA___x552	TL_TLA_TLARS;
...
TLint4	seqNo;
{
	...
	{
	    TLstring	__x565;
	    TL_TLS_TLSVIS((TLint4) 10, (TLint4) 1, (TLint4) 10, __x565);
	    {
		TLstring	__x564;
		TL_TLS_TLSVIS((TLint4) seqNo, (TLint4) 1, (TLint4) 10, __x564);
		{
		    TLstring	__x563;
		    TL_TLS_TLSCAT("Sequence number ", __x564, __x563);
		    {
			TLstring	__x562;
			TL_TLS_TLSCAT(__x563, " passed to \"randnext\" not in range 1..", __x562);
			{
			    TLstring	__x561;
			    TL_TLS_TLSCAT(__x562, __x565, __x561);
			    TL_TLQUIT(__x561, (TLint4) 102);
			};
		    };
		};
	    };
    ...
    if ((seed & 1) != 0) {
	value = seed ^ 0xA205B064;
    } else {
	value = seed ^ 0xBB40DE7D;
    };
    if ((value & 1) == 0) {
	value += 1;
    };
    TL_TLA_TLARS[seqNo] = value;
}
