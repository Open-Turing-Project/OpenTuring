...
typedef	TLnat4	TL_TLA___x477[11];
extern TL_TLA___x477	TL_TLA_TLARS;
...
TLreal8 TL_TLA_TLA8SR (angle)
TLreal8	angle;
{
    if (( TLSIMPLEABS(angle)) > 0.84331486e9) {
	TL_TLQUIT("Argument too large in \"sin\"", (TLint4) 100);
    };
    if (angle < 0) {
	return (TL_TLA_TLA8T((TLreal8) angle, (TLreal8) (- angle), (TLint4) -1));
    } else {
	return (TL_TLA_TLA8T((TLreal8) angle, (TLreal8) angle, (TLint4) 1));
    };
    /* NOTREACHED */
