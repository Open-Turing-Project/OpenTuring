...
typedef	TLaddressint	TL_TLI___x506[21];
typedef	TL_TLI___x506	TL_TLI_ArgList;
typedef	TLchar	TL_TLI___x509[16];
typedef	TL_TLI___x509	TL_TLI___x508[3];
typedef	struct TL_TLI_StreamEntryType	TL_TLI___x510[33];
extern TL_TLI___x510	TL_TLI_TLIS;
...
typedef	TLchar	TL_TLI___x514[42];
extern TL_TLI___x514	TL_TLI_TLIPXL;
typedef	TLchar	TL_TLI___x515[44];
extern TL_TLI___x515	TL_TLI_TLIPXN;
...
extern void TL_TLI_TLIFS ();
void TL_TLI_TLIOP (fileMode, fileName, streamNo)
TLstring	fileMode;
TLstring	fileName;
TLint4	*streamNo;
{
	...
	if (strcmp(fileMode, "w") == 0) {
	    ...
	    typedef	TLchar	__x517[61];
	    __x517	errorMsg;
	    TLSTRASS(60, errorMsg, "Open mode must be one of \"r\" or \"w\" - not \"");
	    if (TL_TLS_TLSLEN(fileMode) > 9) {
		{
		    TLchar	__x519[7];
		    TL_TLS_TLSBXX(__x519, (TLint4) 5, (TLint4) 1, fileMode);
		    {
			TLstring	__x518;
			TL_TLS_TLSCAT(__x519, " ...\"", __x518);
			TLSTRCATASS(errorMsg, __x518, 60);
		    };
		};
	    } else {
		{
		    TLstring	__x520;
		    TL_TLS_TLSCAT(fileMode, "\"", __x520);
		    TLSTRCATASS(errorMsg, __x520, 60);
		};
	    };
	    TL_TLQUIT(errorMsg, (TLint4) 81);
	};
    };
    {
	TL_Cint	firstStream;
	firstStream = TL_TLI_TLIARC + 1;
	if (firstStream > (20 + 1)) {
	    firstStream = 21;
	};
	{
	    ...
	    if (sn >= __x521) {
		for(;;) {
		    TLBINDREG((*streamMode), TL_TLI_StreamModeSet);
		    streamMode = &(TL_TLI_TLIS[sn - -2].mode);
		    if (((*streamMode) == 0) || ((TLSMLSMLSETCONST(15) & (*streamMode)) != 0)) {
			(*streamNo) = sn;
			break;
		    };
		    if (sn == __x521) break;
    ...
    {
	TLBIND((*streamName), TLaddressint);
	TLint4	nameLength;
	register TLaddressint	dst;
	register TLaddressint	src;
	TLaddressint	lastAddr;
	streamName = &(TL_TLI_TLIS[(*streamNo) - -2].fileName);
	nameLength = TL_TLS_TLSLEN(fileName);
	TL_TLB_TLBMAL((TLnat4) (nameLength + 1), &((*streamName)));
	if ((unsigned long) (*streamName) == 0) {
	    (*streamNo) = 0;
	    return;
	};
	dst = (TLaddressint) (*streamName);
	src = (TLaddressint) ((unsigned long)fileName);
	lastAddr = (TLaddressint) ((unsigned long) ((unsigned long)fileName) + nameLength);
	for(;;) {
    ...
    if ((*streamNo) == 0) {
	TL_TLB_TLBMFR((TLaddressint) (TL_TLI_TLIS[sn - -2].fileName));
    };
}
