...
typedef	struct TL_TLM_TL_ConditionDescriptor	TL_TLM___x352[65535];
...
register struct TL_TLM_TL_ConditionDescriptor	*cd;
{
	...
	md->entryQ.tail->monitorQlink = TL_TLKPD;
    } else {
	md->entryQ.head = TL_TLKPD;
    };
    md->entryQ.tail = TL_TLKPD;
    TL_TLK_TLKLKOFF(&(md->mQLock), (TL_TL_lockStatus_t) x);
    pd = cd->signalQ.head;
    cd->signalQ.head = pd->monitorQlink;
