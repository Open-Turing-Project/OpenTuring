...
typedef	struct TL_TLM_TL_ConditionDescriptor	TL_TLM___x360[65535];
...
void TL_TLM_TLMCTWT (waitTimeout, cd)
TLnat4	waitTimeout;
register struct TL_TLM_TL_ConditionDescriptor	*cd;
{
    register struct TL_TLM_TL_MonitorDescriptor	*md;
    TLboolean	timedOut;
    ...
    timedOut = 0;
    if (md->deviceMonitor) {
	TL_TLM_TLMDCTWT((TLnat4) waitTimeout, cd);
	return;
    };
    TL_TLK_TLKLKON(&(md->mQLock), &(x));
    if ((cd->signalQ.head) != ((struct TL_TL_ProcessDescriptor *) 0)) {
	cd->signalQ.tail->monitorQlink = TL_TLKPD;
    } else {
	cd->signalQ.head = TL_TLKPD;
    };
    cd->signalQ.tail = TL_TLKPD;
    TL_TLKPD->monitorQlink = (struct TL_TL_ProcessDescriptor *) 0;
    TL_TLKPD->waitParameter = waitTimeout;
    TL_TLKPD->timeOutStatus = (((TLint4) 0 << 0) & 1) | ((TL_TLKPD->timeOutStatus) & -2);
    TL_TLM_TLMGNEP(md, &(pd));
    if (pd != ((struct TL_TL_ProcessDescriptor *) 0)) {
	TL_TLK_TLKLKOFF(&(md->mQLock), (TL_TL_lockStatus_t) x);
	TL_TLK_TLKSSYNC(pd);
    } else {
	md->entryParameter = 0;
	TL_TLK_TLKLKOFF(&(md->mQLock), (TL_TL_lockStatus_t) x);
    };
    TL_TLK_TLKSTIMO((TLnat4) waitTimeout, &(timedOut));
    if (!timedOut) {
	if ((((TL_TLKPD->timeOutStatus) & 1) >> 0) == 0) {
	    TL_TLQUIT("TLMCTWT: LIBRARY ABORT - timeOutCondSignalled is false.", (TLint4) 21);
	};
	return;
    };
    TL_TLK_TLKLKON(&(md->mQLock), &(x));
    if ((((TL_TLKPD->timeOutStatus) & 1) >> 0) == 1) {
	TL_TLK_TLKLKOFF(&(md->mQLock), (TL_TL_lockStatus_t) x);
	TL_TLK_TLKSSYNC(TL_TLKPD);
    } else {
	if ((cd->signalQ.head) == TL_TLKPD) {
	    cd->signalQ.head = TL_TLKPD->monitorQlink;
	} else {
	    register struct TL_TL_ProcessDescriptor	*last;
	    last = cd->signalQ.head;
	    for(;;) {
		if (last == ((struct TL_TL_ProcessDescriptor *) 0)) {
		    TL_TLQUIT("TLMCTWT: LIBRARY ABORT - TLKPD not found in conditon Q", (TLint4) 21);
		};
		if ((last->monitorQlink) == TL_TLKPD) {
		    break;
		};
		last = last->monitorQlink;
	    };
	    last->monitorQlink = TL_TLKPD->monitorQlink;
	    if ((last->monitorQlink) == ((struct TL_TL_ProcessDescriptor *) 0)) {
		cd->signalQ.tail = last;
	    };
	};
	TL_TLKPD->monitorQlink = (struct TL_TL_ProcessDescriptor *) 0;
	if ((md->entryParameter) == 0) {
	    ...
	    if ((md->reEntryQ.head) != ((struct TL_TL_ProcessDescriptor *) 0)) {
		md->reEntryQ.tail->monitorQlink = TL_TLKPD;
	    } else {
		md->reEntryQ.head = TL_TLKPD;
	    };
	    md->reEntryQ.tail = TL_TLKPD;
	    TL_TLK_TLKLKOFF(&(md->mQLock), (TL_TL_lockStatus_t) x);
	    TL_TLK_TLKSSYNC(TL_TLKPD);
	};
    };
