...
typedef	TLnat4	TL_TLA___x609[11];
extern TL_TLA___x609	TL_TLA_TLARS;
...
extern void TL_TLA_TLAVSI ();
void TL_TLA_TLAVSN (source, base, answer, error, quitOnError)
TLstring	source;
TLnat1	base;
TLnat4	*answer;
TLboolean	*error;
TLboolean	quitOnError;
{
    ...
    value = 0;
    maxValueDivBase = 0xFFFFFFFF / base;
    maxValueModBase = 0xFFFFFFFF % base;
    for(;;) {
	if (((* (TLchar *) src)) != ' ') {
	    break;
	};
    ...
    if (((* (TLchar *) src)) == '\0') {
	if (quitOnError) {
	    TL_TLQUIT("String passed to \"strnat\" is not in correct format", (TLint4) 90);
	};
	(*error) = 1;
	return;
    };
	...
	if ((*error) || (digit >= base)) {
	    if (quitOnError) {
		TL_TLQUIT("Illegal character in string passed to \"strnat\"", (TLint4) 90);
	    };
	    (*error) = 1;
	    return;
	};
	if ((value > maxValueDivBase) || ((value == maxValueDivBase) && (digit > maxValueModBase))) {
	    if (quitOnError) {
		TL_TLQUIT("Overflow in result of \"strnat\"", (TLint4) 50);
	    };
	    (*error) = 1;
	    return;
	};
