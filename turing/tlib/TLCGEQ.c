...
TLboolean TL_TLC_TLCGEQ (sl, sr, ssize)
TLaddressint	sl;
TLaddressint	sr;
TLint4	ssize;
{
    typedef	TLnat1	byteset;
    register TLaddressint	dst;
    register TLaddressint	src;
    dst = (TLaddressint) sl;
    src = (TLaddressint) sr;
    {
	register TLint4	__x323;
	TLint4	__x327;
	__x327 = ssize;
	__x323 = 1;
	if (__x323 <= __x327) {
	    for(;;) {
		if (!(0 == (((* (byteset *) src))  & ~ ((* (byteset *) dst))))) {
		    return (0);
		};
		dst += 1;
		src += 1;
		if (__x323 == __x327) break;
		__x323++;
	    }
	};
    };
