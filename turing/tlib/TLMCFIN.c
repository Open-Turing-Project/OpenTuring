...
void TL_TLM_TLMCFIN (cd)
struct TL_TLM_TL_ConditionDescriptor	*cd;
{
    struct TL_TLM_TL_ConditionDescriptor	*next;
    struct TL_TLM_TL_ConditionDescriptor	*prev;
    if ((cd->signalQ.head) != ((struct TL_TL_ProcessDescriptor *) 0)) {
	TL_TLQUIT("TLMCFIN: LIBRARY ABORT - Condition queue was not nil", (TLint4) 112);
    };
    next = cd->nextCondition;
    prev = cd->prevCondition;
    if (next != ((struct TL_TLM_TL_ConditionDescriptor *) 0)) {
	next->prevCondition = prev;
    };
    if (prev == ((struct TL_TLM_TL_ConditionDescriptor *) 0)) {
	cd->md->firstCondition = next;
    } else {
	prev->nextCondition = next;
    };
}
