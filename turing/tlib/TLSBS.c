...
typedef	TLchar	TL_TLS___x345[3];

extern void TL_TLS_TLSBX ();
typedef	TLchar	TL_TLS___x346[3];
void TL_TLS_TLSBS (target, offset, source)
TL_TLS___x346	target;
TLint4	offset;
TLstring	source;
{
    TL_TLS_TLSBX(target, (TLint4) (TL_TLS_TLSLEN(source) + offset), source);
}
