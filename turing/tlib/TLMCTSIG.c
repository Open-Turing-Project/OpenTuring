...
typedef	struct TL_TLM_TL_ConditionDescriptor	TL_TLM___x347[65535];
...
register struct TL_TLM_TL_ConditionDescriptor	*cd;
{
	...
	if (pd != ((struct TL_TL_ProcessDescriptor *) 0)) {
	    ...
	    pd->waitParameter = 0;
	    pd->timeOutStatus = (((TLint4) 1 << 0) & 1) | ((pd->timeOutStatus) & -2);
	    TL_TLK_TLKSWAKE(pd);
	};
	return;
    };
	...
	if ((md->reEntryQ.head) != ((struct TL_TL_ProcessDescriptor *) 0)) {
	    md->reEntryQ.tail->monitorQlink = pd;
	} else {
	    md->reEntryQ.head = pd;
	};
	md->reEntryQ.tail = pd;
