...
typedef	TLaddressint	TL_TLI___x508[21];
typedef	TL_TLI___x508	TL_TLI_ArgList;
typedef	TLchar	TL_TLI___x511[16];
typedef	TL_TLI___x511	TL_TLI___x510[3];
typedef	struct TL_TLI_StreamEntryType	TL_TLI___x512[33];
extern TL_TLI___x512	TL_TLI_TLIS;
...
typedef	TLchar	TL_TLI___x516[42];
extern TL_TLI___x516	TL_TLI_TLIPXL;
typedef	TLchar	TL_TLI___x517[44];
extern TL_TLI___x517	TL_TLI_TLIPXN;
...
void TL_TLI_TLIPC (putWidth, putItem, __x519, streamNo)
TLint4	putWidth;
TLchar	putItem[];
TLint4	__x519;
TLint2	streamNo;
{
    typedef	TLstring	StringType;
    TLnat4	fd;
    if (putWidth < 0) {
	TL_TLQUIT(TL_TLI_TLIPXN, (TLint4) 67);
    };
    if (putWidth > 255) {
	TL_TLQUIT(TL_TLI_TLIPXL, (TLint4) 79);
    };
    fd = TL_TLI_TLIS[streamNo - -2].info;
    if (__x519 == 1) {
	(*TL_TLI_TLIFPC)((TLchar) (putItem[(0)]), (TLaddressint) fd);
    } else {
	TLint4	dummy;
	dummy = (*TL_TLI_TLIFWR)((TLaddressint) ((unsigned long)putItem), (TL_Cint) 1, (TL_Cint) __x519, (TLaddressint) fd);
    };
    if (putWidth > 0) {
	TLint4	blanks;
	blanks = putWidth - __x519;
	if (blanks > 0) {
	    {
		TLstring	__x520;
		TL_TLS_TLSRPT(" ", (TLint4) blanks, __x520);
		(*TL_TLI_TLIFPS)(__x520, (TLaddressint) fd);
	    };
	};
    };
}
