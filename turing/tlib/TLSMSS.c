...
typedef	TLchar	TL_TLS___x419[3];

extern void TL_TLS_TLSBX ();
typedef	TLchar	TL_TLS___x420[3];

extern void TL_TLS_TLSBXX ();

extern void TL_TLS_TLSCXX ();
void TL_TLS_TLSMSS (target, source, dstIndex)
TLstring	target;
TLstring	source;
TLint4	dstIndex;
{
    ...
    if (dstIndex > ((TL_TLS_TLSLEN(target) - TL_TLS_TLSLEN(source)) + 1)) {
