...
typedef	struct TL_TLM_TL_ConditionDescriptor	TL_TLM___x325[65535];
...
extern void TL_TLM_TLMUDUMP ();
void TL_TLM_TLMGNEP (md, pd)
struct TL_TLM_TL_MonitorDescriptor	*md;
struct TL_TL_ProcessDescriptor	**pd;
{
    if (((md->entryQ.head) == ((struct TL_TL_ProcessDescriptor *) 0)) && ((md->reEntryQ.head) == ((struct TL_TL_ProcessDescriptor *) 0))) {
	(*pd) = (struct TL_TL_ProcessDescriptor *) 0;
    } else {
	if ((md->reEntryQ.head) != ((struct TL_TL_ProcessDescriptor *) 0)) {
	    (*pd) = md->reEntryQ.head;
	    md->reEntryQ.head = (*pd)->monitorQlink;
	} else {
	    (*pd) = md->entryQ.head;
	    md->entryQ.head = (*pd)->monitorQlink;
	};
