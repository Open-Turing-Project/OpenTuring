...
typedef	TLnat4	TL_TLA___x490[11];
extern TL_TLA___x490	TL_TLA_TLARS;
...
TLreal8 TL_TLA_TLA8XP (value)
TLreal8	value;
{
    TLreal8	answer;
    TLint4	error;
    TL_TLA_TLA8X((TLreal8) value, &(answer), &(error));
    switch (error) {
	case 0:
	    break;
	case 1:
	    {
		TL_TLQUIT("Overflow in standard function \"exp\"", (TLint4) 51);
	    }
	    break;
	case 2:
	    {
		if (TL_TLECU) {
		    TL_TLQUIT("Underflow in standard function \"exp\"", (TLint4) 52);
		} else {
		    answer = 0;
		};
	    }
