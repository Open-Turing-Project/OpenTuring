...
typedef	TLaddressint	TL_TLI___x444[21];
typedef	TL_TLI___x444	TL_TLI_ArgList;
typedef	TLchar	TL_TLI___x447[16];
typedef	TL_TLI___x447	TL_TLI___x446[3];
typedef	struct TL_TLI_StreamEntryType	TL_TLI___x448[33];
extern TL_TLI___x448	TL_TLI_TLIS;
...
typedef	TLchar	TL_TLI___x452[42];
extern TL_TLI___x452	TL_TLI_TLIPXL;
typedef	TLchar	TL_TLI___x453[44];
extern TL_TLI___x453	TL_TLI_TLIPXN;
...
TLstring	argString;
{
	...
	{
	    TLstring	__x456;
	    TL_TLS_TLSVIS((TLint4) argNo, (TLint4) 1, (TLint4) 10, __x456);
	    {
		TLstring	__x455;
		TL_TLS_TLSCAT("Argument number ", __x456, __x455);
		{
		    TLstring	__x454;
		    TL_TLS_TLSCAT(__x455, " does not exist", __x454);
		    TL_TLQUIT(__x454, (TLint4) 72);
		};
	    };
	};
    } else {
	if (argNo < 0) {
	    TL_TLQUIT("Negative argument number passed to \"fetcharg\"", (TLint4) 71);
	};
    };
    src = (TLaddressint) ((* (TL_TLI_ArgList *) TL_TLI_TLIARV)[argNo]);
    dst = (TLaddressint) ((unsigned long)argString);
    {
	register TLint4	__x440;
	for (__x440 = 1; __x440 <= 255; __x440++) {
	    if (((* (TLchar *) src)) == '\0') {
		break;
	    };
