...
typedef	TLchar	TL_TLS___x397[3];

extern void TL_TLS_TLSBX ();
typedef	TLchar	TL_TLS___x398[3];

extern void TL_TLS_TLSBXX ();

extern void TL_TLS_TLSCXX ();
void TL_TLS_TLSDEL (target, __x404, delLength, dstIndex)
TLchar	target[];
TLint4	__x404;
TLint4	delLength;
TLint4	dstIndex;
{
    TLint4	srcIndex;
    srcIndex = dstIndex + delLength;
    if (dstIndex <= 0) {
	TL_TLQUIT("Starting index of \"strdelete\" is less than 1", (TLint4) 40);
    };
    if (srcIndex > (TL_TLS_TLSLEN(target) + 1)) {
	TL_TLQUIT("Attempt to \"strdelete\" past the string length", (TLint4) 40);
    };
    if (delLength > 0) {
	register TLaddressint	dst;
	register TLaddressint	src;
	dst = (TLaddressint) (((unsigned long) ((unsigned long)target) + dstIndex) - 1);
	src = (TLaddressint) (((unsigned long) ((unsigned long)target) + srcIndex) - 1);
	for(;;) {
