...
typedef	TLchar	TL_TLS___x461[3];

extern void TL_TLS_TLSBX ();
typedef	TLchar	TL_TLS___x462[3];
...
TLstring	target;
{
    ...
    register TLint2	count;
    register TLaddressint	dst;
    if (width < 0) {
	TL_TLQUIT("Negative width passed to \"natstr\"", (TLint4) 67);
    };
    if (width > 255) {
	TL_TLQUIT("String generated by \"natstr\" is too long", (TLint4) 93);
    };
    if ((base < 2) || (base > 36)) {
	TL_TLQUIT("Illegal conversion base passed to \"natstr\"", (TLint4) 106);
    };
    ...
    {
	register TLint4	__x456;
	for (__x456 = width - count; __x456 >= 1; __x456--) {
	    (* (TLchar *) dst) = ' ';
	    dst += 1;
	};
    };
