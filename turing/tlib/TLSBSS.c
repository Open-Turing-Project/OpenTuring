...
typedef	TLchar	TL_TLS___x355[3];

extern void TL_TLS_TLSBX ();
typedef	TLchar	TL_TLS___x356[3];
...
{
    TLint4	srcLength;
    srcLength = TL_TLS_TLSLEN(source);
    TL_TLS_TLSBXX(target, (TLint4) (srcLength + endOffset), (TLint4) (srcLength + startOffset), source);
}
