...
typedef	TLchar	TL_TLS___x411[3];

extern void TL_TLS_TLSBX ();
typedef	TLchar	TL_TLS___x412[3];

extern void TL_TLS_TLSBXX ();

extern void TL_TLS_TLSCXX ();
void TL_TLS_TLSMCS (target, __x421, source, dstIndex)
TLchar	target[];
TLint4	__x421;
TLstring	source;
TLint4	dstIndex;
{
    ...
    if (dstIndex > ((__x421 - TL_TLS_TLSLEN(source)) + 1)) {
