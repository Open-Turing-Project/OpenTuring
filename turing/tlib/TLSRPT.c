...
typedef	TLchar	TL_TLS___x434[3];

extern void TL_TLS_TLSBX ();
typedef	TLchar	TL_TLS___x435[3];
...
TLstring	target;
{
    ...
    if ((count <= 0) || (((* (TLchar *) ((unsigned long)source))) == '\0')) {
	(* (TLchar *) dst) = '\0';
	return;
    };
    if ((count > 255) || ((count * TL_TLS_TLSLEN(source)) > 255)) {
	TL_TLQUIT("Repeat function produced string too long", (TLint4) 93);
    };
    {
	register TLint4	__x428;
	for (__x428 = count; __x428 >= 1; __x428--) {
