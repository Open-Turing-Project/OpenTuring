...
typedef	TLnat4	TL_TLA___x537[11];
extern TL_TLA___x537	TL_TLA_TLARS;
...
TLreal8 TL_TLA_TLAPRI (right, left)
TLint4	right;
TLreal8	left;
{
    register TLreal8	product;
    if (left == 0) {
	if (right == 0) {
    ...
    if (right < 0) {
	{
	    register TLint4	__x530;
	    for (__x530 = right; __x530 <= -1; __x530++) {
		product /= left;
	    };
	};
    } else {
	{
	    register TLint4	__x531;
	    for (__x531 = right; __x531 >= 1; __x531--) {
		product *= left;
	    };
	};
    };
    if (product == 0) {
	if (TL_TLECU) {
	    TL_TLQUIT("Underflow in \"**\" operation", (TLint4) 52);
	};
    } else {
	TLnat4	hiOrder;
	TLnat4	loOrder;
	TL_TLA_TLAV8D((TLreal8) product, &(hiOrder), &(loOrder));
	if (((hiOrder & 2146435072) >> 20) == 2047) {
	    TL_TLQUIT("Overflow in \"**\" operation", (TLint4) 51);
	};
    };
    return (product);
    /* NOTREACHED */
