...
typedef	TLchar	TL_TLS___x437[3];

extern void TL_TLS_TLSBX ();
typedef	TLchar	TL_TLS___x438[3];

extern void TL_TLS_TLSBXX ();

extern void TL_TLS_TLSCXX ();
void TL_TLS_TLSVCS (source, __x452, target)
TLchar	source[];
TLint4	__x452;
TLstring	target;
{
    ...
    if (__x452 > 255) {
	TL_TLQUIT("Char array is too large to convert to string", (TLint4) 93);
    };
    dst = (TLaddressint) ((unsigned long)target);
    src = (TLaddressint) ((unsigned long)source);
    lastAddr = (TLaddressint) ((unsigned long)&(source[((__x452 + 0) - 1)]));
    for(;;) {
