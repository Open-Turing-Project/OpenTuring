...
typedef	TLaddressint	TL_TLI___x449[21];
typedef	TL_TLI___x449	TL_TLI_ArgList;
typedef	TLchar	TL_TLI___x452[16];
typedef	TL_TLI___x452	TL_TLI___x451[3];
typedef	struct TL_TLI_StreamEntryType	TL_TLI___x453[33];
extern TL_TLI___x453	TL_TLI_TLIS;
...
typedef	TLchar	TL_TLI___x457[42];
extern TL_TLI___x457	TL_TLI_TLIPXL;
typedef	TLchar	TL_TLI___x458[44];
extern TL_TLI___x458	TL_TLI_TLIPXN;
...
extern void TL_TLI_TLIFS ();
void TL_TLI_TLIGC (getWidth, getItem, __x459, streamNo)
TLint4	getWidth;
TLchar	getItem[];
TLint4	__x459;
TLint2	streamNo;
{
    ...
    if (getWidth > __x459) {
	TL_TLQUIT("Get item width greater than item size", (TLint4) 74);
    };
    if (getWidth == 0) {
	return;
    };
    stream = &(TL_TLI_TLIS[streamNo - -2]);
    dst = (TLaddressint) ((unsigned long)getItem);
    lastAddr = (TLaddressint) ((unsigned long) ((unsigned long)getItem) + getWidth);
    if ((*stream).atEof) {
	for(;;) {
	    (* (TLchar *) dst) = '\0';
	    dst += 1;
