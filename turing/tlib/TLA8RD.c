...
typedef	TLnat4	TL_TLA___x466[11];
extern TL_TLA___x466	TL_TLA_TLARS;
...
TLint4 TL_TLA_TLA8RD (value)
TLreal8	value;
{
    TLint4	answer;
    if (value < 0) {
	if ((value + 0.5) < -2147483647) {
	    TL_TLQUIT("Integer overflow in \"round\"", (TLint4) 50);
	};
	if (value <= -2147483647) {
	    return (-2147483647);
	};
    } else {
	if ((value - 0.5) >= 0x7FFFFFFF) {
	    TL_TLQUIT("Integer overflow in \"round\"", (TLint4) 50);
	};
    };
    answer = TL_TLA_TLA8FL((TLreal8) value);
    if ((value - answer) >= 0.5) {
	answer += 1;
    };
    return (answer);
