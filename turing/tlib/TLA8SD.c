...
typedef	TLnat4	TL_TLA___x472[11];
extern TL_TLA___x472	TL_TLA_TLARS;
...
TLreal8 TL_TLA_TLA8SD (angle)
TLreal8	angle;
{
    TLreal8	radAngle;
    radAngle = angle * 0.0174532925199432957692366;
    if (( TLSIMPLEABS(radAngle)) > 0.84331486e9) {
	TL_TLQUIT("Argument too large in \"sind\"", (TLint4) 100);
    };
    if (radAngle < 0) {
	return (TL_TLA_TLA8T((TLreal8) radAngle, (TLreal8) (- radAngle), (TLint4) -1));
    } else {
	return (TL_TLA_TLA8T((TLreal8) radAngle, (TLreal8) radAngle, (TLint4) 1));
    };
    /* NOTREACHED */
