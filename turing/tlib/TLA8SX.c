...
typedef	TLnat4	TL_TLA___x412[11];
extern TL_TLA___x412	TL_TLA_TLARS;
...
TLreal8 TL_TLA_TLA8SX (source, expValue)
TLreal8	source;
TLint4	expValue;
{
    TLnat2	exponent;
    ...
    if (expValue < (-1022)) {
	if (TL_TLECU) {
	    TL_TLQUIT("Result of \"setexp\" underflows", (TLint4) 52);
	};
	return (0);
    } else {
	if (expValue > (1024)) {
	    TL_TLQUIT("Result of \"setexp\" overflows", (TLint4) 51);
	} else {
	    exponent = (expValue + 1023) - 1;
	};
    };
    TL_TLA_TLAV8D((TLreal8) source, &(hiOrder), &(loOrder));
    if (((TL_TLECU && (exponent == 0)) && (loOrder == 0)) && (((hiOrder & 1048575) >> 0) == 0)) {
	TL_TLQUIT("Result of \"setexp\" underflows", (TLint4) 52);
    };
    hiOrder = (((TLint4) exponent << 20) & 2146435072) | (hiOrder & -2146435073);
