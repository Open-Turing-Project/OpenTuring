...
typedef	TLchar	TL_TLS___x378[3];

extern void TL_TLS_TLSBX ();
typedef	TLchar	TL_TLS___x379[3];

extern void TL_TLS_TLSBXX ();
void TL_TLS_TLSCXX (source, __x381, startIndex, endIndex, target)
TLchar	source[];
TLint4	__x381;
TLint4	startIndex;
TLint4	endIndex;
TLstring	target;
{
    ...
    if (startIndex <= 0) {
	TL_TLQUIT("Left bound of substring is less than 1", (TLint4) 40);
    };
    if ((startIndex - endIndex) > 1) {
	TL_TLQUIT("Left bound of substring exceeds right bound by more than 1", (TLint4) 92);
    };
    if (endIndex > __x381) {
	TL_TLQUIT("Right bound of substring greater than array size", (TLint4) 40);
    };
    if ((endIndex - startIndex) >= 255) {
	TL_TLQUIT("Substring too large to fit into string", (TLint4) 93);
    };
    dst = (TLaddressint) ((unsigned long)target);
    src = (TLaddressint) ((unsigned long)&(source[(startIndex - 1)]));
    lastAddr = (TLaddressint) ((unsigned long)&(source[(endIndex - 1)]));
    for(;;) {
	if ((unsigned long) src > (unsigned long) lastAddr) {
	    break;
	};
	if ((((* (TLnat1 *) src)) & 127) == 0) {
	    TL_TLQUIT("Illegal character in substring", (TLint4) 43);
	};
