		     ...
		     const int color)
{
	...
	for (cp = legend; *cp; cp++)
	    ...
	    {
		if (AsciiTable[*cp][i] & (1 << (GIF_FONT_WIDTH - j)))
...
void DrawRectangle(SavedImage *Image,
		     ...
		     const int color)
{
    char *bp = Image->RasterBits + Image->ImageDesc.Width * y + x;
    int i;
		     ...
		     const int bg, const int fg)
{
    ...
    DrawRectangle(Image,
	    ...
	    border + LineCount * GIF_FONT_HEIGHT + border - 1,
	    bg);
    ...
    DrawBox(Image,
	    ...
	    border + LineCount * GIF_FONT_HEIGHT + border,
	    fg);
}
