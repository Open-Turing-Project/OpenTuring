% The "View.Update Example" program.
...
% Now, any drawing to the screen won't appear until a View.Update is given.
loop
    cls
    for i : 1 .. NUM_BALLS
	if x (i) + dx (i) < RADIUS or x (i) + dx (i) > maxx - RADIUS then
	    dx (i) := -dx (i)
	end if
	if y (i) + dy (i) < RADIUS or y (i) + dy (i) > maxy - RADIUS then
	    dy (i) := -dy (i)
	end if
	x (i) := x (i) + dx (i)
	y (i) := y (i) + dy (i)
